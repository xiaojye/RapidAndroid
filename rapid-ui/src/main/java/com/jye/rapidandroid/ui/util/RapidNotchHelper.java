 ...
 */
public class RapidNotchHelper {

    private static final String TAG = RapidNotchHelper.class.getSimpleName();
    ...
    public static boolean has3rdNotch(Context context){
        if (RapidDeviceUtils.isHuawei()) {
            return hasNotchInHuawei(context);
        } else if (RapidDeviceUtils.isVivo()) {
            return hasNotchInVivo(context);
        } else if (RapidDeviceUtils.isOppo()) {
            return hasNotchInOppo(context);
        } else if (RapidDeviceUtils.isXiaomi()) {
    ...
    private static Rect get3rdSafeInsetRect(Context context){
        // 全面屏设置项更改
        if (RapidDeviceUtils.isHuawei()) {
            boolean isHuaweiNotchSetToShow = RapidScreenUtils.huaweiIsNotchSetToShowInSetting(context);
    ...
    private static Rect getRectInfoRotation0(Context context) {
        Rect rect = new Rect();
        if (RapidDeviceUtils.isVivo()) {
            // TODO vivo 显示与亮度-第三方应用显示比例
            rect.top = getNotchHeightInVivo(context);
            rect.bottom = 0;
        } else if (RapidDeviceUtils.isOppo()) {
            // TODO OPPO 设置-显示-应用全屏显示-凹形区域显示控制
            rect.top = RapidStatusBarUtils.getStatusbarHeight(context);
            rect.bottom = 0;
        } else if (RapidDeviceUtils.isHuawei()) {
    ...
    private static Rect getRectInfoRotation90(Context context) {
        ...
        if (RapidDeviceUtils.isVivo()) {
            ...
            rect.left = RapidStatusBarUtils.getStatusbarHeight(context);
            rect.right = 0;
        } else if (RapidDeviceUtils.isHuawei()) {
    ...
    private static Rect getRectInfoRotation180(Context context) {
        ...
        if (RapidDeviceUtils.isVivo()) {
            ...
            rect.top = 0;
            rect.bottom = RapidStatusBarUtils.getStatusbarHeight(context);
        } else if (RapidDeviceUtils.isHuawei()) {
            int[] notchSize = getNotchSizeInHuawei(context);
            rect.top = 0;
    ...
    private static Rect getRectInfoRotation270(Context context) {
        ...
        if (RapidDeviceUtils.isVivo()) {
            ...
            rect.right = RapidStatusBarUtils.getStatusbarHeight(context);
            rect.left = 0;
        } else if (RapidDeviceUtils.isHuawei()) {
            if (sHuaweiIsNotchSetToShow) {
    ...
    public static int getNotchHeightInXiaomi(Context context) {
        ...
        return RapidScreenUtils.getStatusBarHeight(context);
    }

    public static int getNotchWidthInVivo(Context context){
        return RapidScreenUtils.dp2px(context, 100);
    }

    public static int getNotchHeightInVivo(Context context){
        return RapidScreenUtils.dp2px(context, 27);
    }
    ...
    public static boolean needFixLandscapeNotchAreaFitSystemWindow(View view){
       return (RapidDeviceUtils.isXiaomi() || RapidDeviceUtils.isVivo()) && RapidNotchHelper.hasNotch(view);
    }
